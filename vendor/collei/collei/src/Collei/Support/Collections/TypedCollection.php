...
class TypedCollection extends Collection implements Iterator
{
	...
	public function add($element)
	{
		if (!is_a($element, $this->enforcedType)) {
			throw new InvalidArgumentException(
				'Argument is not an instance of ' . $this->enforcedType
			);
		}
		//
		$this->data[] = $element;
	 ...
	 *	@return	instanceof \Collei\Support\Collections\TypedCollection
	 */
	public static function fromTypedArray(
		array $array = null, string $enforcedType, bool $ignoreError = true
	) {
		...
		if (!is_null($array)) {
			foreach ($array as $n => $v) {
				if (is_object($v)) {
					if (
						get_class($v) === $enforcedType
						|| is_subclass_of($v, $enforcedType)
						|| in_array($enforcedType, class_implements($v))
					) {
						$collection->data[] = $v;
						...
						throw new InvalidArgumentException(
							'Array passed to TypedCollection::fromArray has '
